...
Tổng quan mô hình decorator: https://viblo.asia/p/decorator-design-pattern-tro-thu-dac-luc-cua-developers-1VgZvQ1OKAw
...
Lợi ích of decorators: https://javascript.info/call-apply-decorators#transparent-caching
...
cách hoạt động/tác dụng: observing property decorator declared in the class at compiling/runtime để limit khả năng of property nếu nó không thỏa dk.
...
cách hoạt động/tác dụng: observing parameter decorator declared at compiling/runtime để limit khả năng of parameter nếu nó không thỏa dk.
...
1/decorator Func
2/decorator factory: 1 Func that wraps fn decorator and then returns fn decorator như đã biết (at v2-4ms43ss) -> tức là custom 1 fn decorator: https://www.typescriptlang.org/docs/handbook/decorators.html#decorator-factories

Decorator factory còn là 1 closure in ts/js
...
4ms31ss: cách Multiple decorators hoạt động
...
vd: 1 class's field (properties, methods, ...vv) dùng Multiple decorators, see more at xx9
...
2ms21ss: Lưu ý cách calling/hoạt động của 1 decorator fn at runtime:

1/at runtime thì tự động calling a decorator fn via '@'. Vd: @logClass

2/at runtime, a decorator có '()' theo sau thì nghĩa là ta đang dùng 1 decorator factory cho dù có param hay ko in '()'. Vd: @logClass(); @classDecor('class_name')

như đã nói (v3-2ms41ss), Decorator factory là 1 closure -> sau 1st runtime (turn on debug/F5) thì Decorator factory return 1 decorator fn, và fn này chỉ được triggerd (2nd running) from client's actions (http requests) thì nó mới thực sự calling/executing to class's decorator
...
4ms01ss: dùng launch.json file để runtime by debugging decorators -> ok like ad, see .vscode\launch.json

4ms34ss: thêm 1 '@logMethod' cho this property để trở thành multiple decorators, see app.ts

-> in multiple decorators, các decorator expressions được evaluated từ trên xuống dưới, then được called từ dưới lên trên như đã biết, see more xx9
*/
...
-> test debug runtime in vsCode: Server is runtime on port 3000 -> ok like ad
...
1/turn on debug in vsCode -> Server is runtime on port 3000
...
-> APIServer file chứa cấu hình 1 server để runtime và tạo kết nối cho client
...
if (!objectProperties.includes.... -> in this case, do là 1st runtime nên 'objectProperties' sẽ là '[]', nên mới có expression này để tạo/chứa a new key list

dùng 'defineMetadata' để define objectProperties (a new property list) to entity for a store/use in next time.

xx13: nhận ra, nếu 'getMetadata' sử dụng 1 new 'metadataKey' (tức 1st time) thì 'objectProperties' sẽ luôn là 1 empty [] -> then, go through logic, 'metadataKey' defined/stored by 'defineMetadata' for store/use. In next time, 'getMetadata' sử dụng lại this 'metadataKey' thì 'objectProperties' sẽ có items, see more xx3
...
1/runtime debug -> check in DEBUG CONSOLE
...
xx12: Lưu ý, class route method's args positions are defaultly located in provided route fns (Request/Response param); nên ta chỉ việc nhìn theo nó mà sử dụng, chứ ko thể thay đổi positions, see log.ts + EntityRouter.ts
...
xx7: lỗi: Cannot read properties of undefined (reading 'ip') -> do starting/1st runtime thì req/res là 'undefined' nên báo lỗi at debugging -> ràng buộc kỹ hơn: if (req != undefined && res != undefined) {..., see log.ts

-> access ip from Express ok.

xx1: nhưng mới phát hiện ra, lỗi tại xx7 là do '@logRoute' trên 1 fn ko có param, hoặc có params thì ko liên quan đến req/res param, dẫn đến decorator tự động accessing to class's method, và nó tìm ko thấy req/res param nên báo 'undefined' at 1st runtime/starting, then dẫn đến 1 chuỗi lỗi như đã biết, see EntityRouter.ts
...
-> cụ thể ở đây ta tạo router instance for to Person class hoặc other class. HoặC cấu hình other routers for authentication, oauth2, share, payment, ...vv
*/
...
32ss: thêm các validator decorators (required, length, isEmail, ...) cho Person's props, see Person.ts

-> tạo validation file, và dùng 'validator' npm; 'reflect-metadata' npm để cấu hình các validators, see validation.ts

1m59ss: dùng 'type' (at xx8) để tạo 1 fn mà sau này sẽ được sử dụng như 1 Validation Fn (as a callback) để thông báo -> type ValidationFunction..., see validation.ts

- is passed: return void; -> no error
- is error: return a message string -> got error

2ms18ss: interface ValidationRule {... -> rules cho Validation Fn đã defined ở trên, see validation.ts

2ms30ss: cấu hình các validator decorators. Giải thích chi tiết, see validation.ts

addValidation(target, propertyKey, ... -> mỗi validator (a decorator fn) sẽ call a addValidation fn, 2 thứ cần xử lý:

1/các addValidation fns dùng validator's defaulted params passed in để thực hiện tương tác giữa decorator fn với reflect-metadata như đã biết

2ms55ss: 2/ Một số validator là decorator factory (length, isInteger -> vì cho ta bổ sung thêm các param options to decorator fn và và return the decorator, ôn v4-2ms21ss), see validation.ts

3ms13ss: function addValidation(... -> nơi tổng thể xử lý decorator fns, messages thông báo khi có lỗi như đã nói, see validation.ts

4ms09ss: let validators: ValidationRule[] = ... -> tạo 1 new validators list via props và tuân theo interface's rules.

let validationRule = { ...
validators.push(...
Reflect.defineMetadata(...

-> cấu hình từng new validator being passing và push to arr. Then, save/store back to arr by 'defineMetadata' để có data sử dụng về sau.

xx3: nhận ra, at runtime thì reflect-metadata get all decorator metadata into a list rồi mới xử lý từng decorator nên ta mới có thể get/define/...vv a list, see validation.ts + ôn xx13

xx9: như đã biết (at v3-4ms31ss) về multiple decorators -> xem qua các console.log để hiểu rõ steps, see validation/entity/Person.ts

cũng như hiểu tổng thể các field decorators are evaluated từ trên xuống, từ trong fn/class ra ngoài fn/class

4ms23ss: cấu hình các VALIDATOR FUNCTIONS tương ứng mà ta đã cấu hình chúng như params, see validation.ts

-> dùng 'validator' npm để sử dụng các validations from library  see validation.ts

5ms33ss: export function validate(..., see validation.ts

-> sau khi cấu hình/defined/stored các validators to entiy thì ta cần 1 fn cung cấp a map of those validators processed back to router (EntityRouter.js) để báo lỗi nếu có.

6ms34ss: dùng validate fn trên cho EntityRouter ->  nếu có err message thì return luôn, see EntityRouter.ts

dùng validate fn cho update fn, see EntityRouter.ts
...
-> ok like ad, ta đã validate dựa trên decorator metadata defined.

xx14: sau khi triggering routes from postAPeople/updateAPeople thì program get all message err rồi mới return báo lỗi -> fix bằng cách return ngay, see validation.ts

-> nhưng nên '//' để kiểu cũ vì đang học.
*/
...
const base64Auth = (... -> get authentication data from header http

const strauth = Buffer... -> decode base64 to string, tức là mã hóa mã-64 thành các strings thông thường cho việc check.
