...
class EntityRouter {
    ...
    createEntity(req, res) {
        ...
        let errorMap = decorators_1.validate(newEntity); //	6ms34ss
        ...
        // const idProperty = Reflect.getMetadata('entityTai:id', newEntity);
        // newEntity[idProperty] = uuid.v4();
    ...
    updateEntity(req, res) {
        ...
        let errorMap = decorators_1.validate(updatedObj); // 6ms34ss
        if (Object.keys(errorMap).length > 0) {
            const output = { errors: errorMap };
            res.status(400).json(output);
            return;
...
], EntityRouter.prototype, "addEntityRoutes", null);
__decorate([
    decorators_1.logRoute,
    __metadata("design:type", Function),
