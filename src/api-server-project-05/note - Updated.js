...
in (the) morning, on friday, the fourteenth of July/ on 14th-Jul in 2023
...
4ms46ss: -> khi làm việc với các generic types in libs, fw, project, ... thì chỉ cần nhớ input'stype param in '<type-param-name>', từ đó bạn sẽ xử lý logic 1 cách hợp lý dựa trên generic type param of input/output đó.
...
-> reflect-metadata cho phép CRUD to the entities (class) for modifies, giống như dùng ORMs (sequelize, Prisma, TypeORM, ...) để CRUD to db.
...
-> xx10: 'metadataKey' do ta tự đặt/custom metadataKey theo rule <name_u_want:passing_key_name>, miễn sao chúng đồng bộ to all related, và quan trọng nhất là naming sao cho coders hiểu fn đó có chức năng gì, see api-server-project-04 (entity.ts, BaseEntity.ts, EntityRouter.ts, ...) + xx15 (hiểu rõ b/c vẫn chỉ là getting class's props) 
...
-> 'id' cũng là 1 property in class, nhưng ta dùng id decorator (a isolated decorator) để định nghĩa/xử lý logic riêng cho nó, nên nó khác với các properties (dùng persist decorator) kia. -> hiểu cách truy cập/xử lý các isolated decorators
...
xx8: 'type' là 1 Type Aliase: chấp nhận any types, thường dùng cho generic vars/fns, see BaseEntity.ts + (project 4 - validation.ts) 
...
xx11: export type EntityTypeInstance<T> = new (...args: any[]) => T: là 'narrow variable generic' for types of input/output in decorator, see BaseEntity.ts, ôn (v3-1m13ss)

new -> đại diện 1 "static" part of a class/function, nghĩa là ta có thể tạo instance of T như 1 class/object/arr/... -> cho nên ko thể dùng các const/var/let 

- https://stackoverflow.com/questions/48731626/new-args-t-syntax

(...args: any[]) -> the Rest operator/parameters of an arr nên có thể dùng bao nhiêu agrs cũng được, do các arr args types là unknown nên sẽ dùng type of any[] 

https://stackoverflow.com/questions/50726326/how-to-go-about-understanding-the-type-new-args-any-any

=> tóm lại: expression này là 1 generic input/output, nó nhận tất cả agrs và any type, vì là 1 instance nếu có thể khởi tạo thông qua 'new()', see BaseEntity.ts
...
/*
git add .
git commit -m"Decorators with API Server; eflect-metadata npm; custom metadataKey; isolated decorators; custom decorators; Type Aliase; narrow variable generic in decorator; 'new' key; (...args: any[]) Rest parameters; access ip from Express; validator/Auth decorator"
git push
